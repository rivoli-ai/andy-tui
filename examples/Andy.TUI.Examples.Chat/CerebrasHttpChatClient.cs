...
public sealed class CerebrasHttpChatClient
{
    ...
    public CerebrasHttpChatClient(ChatConfiguration config)
    {
        ...
        _maxTokens = config.MaxTokens ?? 512;
        _httpClient = new HttpClient();
        _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _apiKey);
        _httpClient.Timeout = TimeSpan.FromSeconds(120);
    }

    public async Task<string> CreateCompletionAsync(IReadOnlyList<Andy.TUI.Examples.Chat.ChatMessage> messages, CancellationToken cancellationToken = default)
    {
